% ==========================================================================
% Ported to the reliability-commons paper template (v0.1) on 2026-07-02.
% STANDARD article class, deliberately NOT venue-specific: draft here, then
% port to the venue class (IEEEtran / Springer / ...) at submission.
%
% Figure rule (portfolio honest-audit culture):
%   Every figure is produced by a figures/*.R script that reads an on-disk
%   result JSON/CSV under ../experiments/results/. Figures regenerate ONLY
%   from those files -- never from hardcoded arrays. Build with
%   `make figures` / `make paper`.
%
% Bibliography: \bibliography{shared,refs} -- `make bib` copies the portfolio
% shared.bib next to this file; refs.bib holds paper-specific entries.
%
% Every quantitative claim carries a "% source:" comment pointing at the
% result JSON it is read from. Numbers change ONLY by re-running analysis.
% Superseded pre-template draft: archive/paper-2026-07-01-pre-template.tex.
% ==========================================================================
...
\usepackage{placeins} % \FloatBarrier: keeps the Fig. 1 overview pinned at the end of the
                       % Introduction instead of drifting into the draft's existing float backlog

\graphicspath{{figures/}}
\captionsetup{font=small,labelfont=bf}
\setlength{\parskip}{0.4em}

\title{\textbf{Geographic coverage debt of frozen Earth-observation foundation models:\\
an encoder-robustness reliability diagnostic and a conditional target-recalibration remedy}}
\author{Zeyu Fu\,\orcidlink{0009-0001-8329-0108}\thanks{%
    Correspondence: \href{mailto:fuzeyu09@gmail.com}{fuzeyu09@gmail.com}.}}
\affil{State Key Laboratory of Trauma and Chemical Poisoning, Institute of Combined Injury, Army
...
for the FNR analysis; five on EuroSAT-MS \citep{helber2019eurosat} and non-European m-so2sat; the EuroSAT
...
(\S\ref{sec:reportcard}): for any frozen GFM, it returns four practitioner-facing numbers before
...
Table~\ref{tab:reportcard}). A Sentinel-2 tile (EuroSAT-MS / GEO-Bench BigEarthNet-S2 / m-so2sat) is
...
would miss. The preregistered universal-restoration
...
see Limitations). This cut also induces a class-frequency (label-prior) shift
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/results.json (accuracy in_dist 0.9524-0.9861; was misstated as 0.95-0.97 in the pre-template draft)
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/results.json (accuracy: 0.9524->0.8328 prithvi, 0.9724->0.8799 ssl4eo, 0.9777->0.9182 clay; ece real_shift 0.1087/0.0757/0.0650/0.0566)
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/results.json (integrated coverage-gap debt, trapezoid over alpha in {0.05,0.10,0.20}: prithvi 8.0e-3, ssl4eo 3.3e-3, ssl4eo_mae 1.4e-3, clay 0.8e-3)
% source: experiments/results/eurosat_stage2_multifm_multialpha/DOFA-SUMMARY-corrected-2026-07-13.json (CORRECTED 768-d embedding; shift_split_cov 0.8507 at alpha 0.05/0.10/0.20; trapezoid gap-integral D approx 6.2e-3, second-heaviest: Prithvi 8.0 > DOFA 6.2 > SSL4EO-DINO 3.3; supersedes buggy 45-d-head-logit DOFA-SUMMARY-2026-07-12.json)
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/results.json (alpha 0.05: split 0.8330/0.8799/0.8984/0.9182; mondrian 0.9547/0.9560/0.9621/0.9602; set size 1.619/1.434/1.393/1.223)
% source: experiments/results/eurosat_stage2_multifm_multialpha/DOFA-SUMMARY-corrected-2026-07-13.json (CORRECTED 768-d embedding; dofa 5th encoder, alpha 0.05/0.10/0.20: indist split 0.9655; shift split 0.8507; shift+mondrian 0.9506/0.9135/0.8730 at set size 1.5839/1.2875/1.0807; restored 2/3 (alpha 0.05,0.10) by paired-seed CI-excludes-0; overall_verdict KILL = multi-FM gate artifact, clay/prithvi absent from single-FM call; supersedes buggy 45-d-head-logit DOFA-SUMMARY-2026-07-12.json)
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/results.json (real_shift ece raw 0.109/0.076/0.065/0.057 -> ece_tempscaled_srcT 0.058/0.041/0.054/0.032; in_dist ece 0.026/0.014/0.005/0.013, 5-seed means)
...
% source: experiments/results/eurosat_weighted_conformal_baseline/results.json (alpha=0.05 coverage split/weighted/target_global and ESS frac; weighted = Tibshirani 2019 covariate-shift split conformal, density-ratio weights from a logistic src-vs-tgt discriminator on the frozen embeddings)
...
\midrule
% source: experiments/results/eurosat_weighted_conformal_baseline/results.json (encoders.<fm>.<alpha>: split/weighted/target_global/w_ess_frac means). alpha=0.05 row reproduces the paper's original headline cell verbatim; alpha=0.10/0.20 rows expand the same frozen record.
...
\midrule
% source: experiments/results/labelshift_adjusted_baseline/results.json (fm_results.<fm>.cells.<alpha>.real_shift: conformal_split/conformal_labelshift_oracle/conformal_labelshift_bbse/conformal_target_global coverage means). alpha=0.05 row reproduces the paper's original headline cell verbatim; alpha=0.10/0.20 rows expand the same frozen record.
...
The through-line is that a conformal audit of a frozen GFM earns its keep exactly where accuracy and
calibration go silent: on the multi-label FNR risk-control guarantee, every one of thirteen frozen
encoders incurs a geographic FNR debt under shift ($2.6$--$4.7\times$ nominal) that only a labeled target
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/recompute_class_prior_balanced_control_2026-07-01.json
...
\midrule
% source: experiments/results/eurosat_stage2_multifm_multialpha/recompute_class_prior_balanced_control_2026-07-01.json (results.unbalanced / results.balanced)
...
\midrule
% source: experiments/results/eurosat_stage2_multifm_multialpha/recompute_class_prior_balanced_control_2026-07-01.json (bigearthnet_perlabel_balanced_control_alpha_0.10: mean_per_label_debt, mondrian_restores_CI_excludes_0; previously surfaced only in Discussion prose, now tabulated)
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/recompute_class_prior_balanced_control_2026-07-01.json (bigearthnet_perlabel_balanced_control_alpha_0.10: prithvi 0.1166->0.1127, ssl4eo 0.2385->0.2364, clay 0.1634->0.1610)
...
accuracy claim is weaker than a five-encoder pilot suggested, and our geographic reach is Europe plus one
...
audit's scope boundaries---single-sensor, single-continent until the So2Sat extension, and encoder-roster
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/results.json (gate: clay_restores_ge2_alphas=false, prithvi_restores_ge2_alphas=true, overall_verdict=KILL)
...
The headline shifts are cross-region (single-sensor Sentinel-2), not
...
A preliminary cross-sensor probe (Sentinel-2 calibration transferred to a native Sentinel-1 encoder on
...
% source: experiments/results/xsensor_real_calib/results_ssl4eo_s1_novariance.json (in_dist_s2 accuracy 0.513 on the identical paired m-so2sat subset; the anomalous alignment-arm run reports 0.060, at 17-class chance)
...
(debug record archived in the code-and-data deposit). We make no cross-sensor claim here and leave
a contrastively-aligned S1/S2 encoder, the right vehicle for that question, to future work. Disentangling
...
% source: experiments/results/eurosat_stage2_multifm_multialpha/DOFA-SUMMARY-corrected-2026-07-13.json (CORRECTED 768-d: shift split 0.8507 -> mondrian 0.9506/0.9135/0.8730 at alpha 0.05/0.10/0.20, restored 2/3 by paired CI; D approx 6.2e-3, second-heaviest; supersedes buggy 45-d DOFA-SUMMARY-2026-07-12.json)
% source: experiments/results/geobench_battery/m-brick-kiln/results.json (dofa cell used the SAME pre-correction 45-d head-logit features; NOT re-runnable locally on 2026-07-13 (GEO-Bench v1.0 m-brick-kiln data absent on the correction host); DOFA m-brick-kiln numbers WITHHELD pending re-computation)
...
archived at Zenodo under DOI \texttt{10.5281/zenodo.21130299}; this DOI is currently reserved on a draft
deposition and becomes active when the record is published.
